"""What the SOLO run shipped, verbatim from `solo-20260916-204608`.

The control: one session, Agent disallowed, nobody to check the work. It
still passes all four constraints. Kept here, rather than merged into
demo/target/pricing.py, so the task stays unsolved for the next run.

The three shipped implementations differ only at the margins -- this one
is the most minimal of them. See shipped_hub.py and shipped_flat.py.
"""
